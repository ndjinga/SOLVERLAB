...
std::vector<double> initialVelocity(double x, double y){
	...
	vtheta = 0;
	vr = 0;
	vec[0] =    cos(theta) * vtheta + sin(theta) * vr  ;
	vec[1] =  - sin(theta) * vtheta + cos(theta) * vr  ;
	
	/* vec[0] =   	 - sin(theta) * vtheta + cos(theta) * vr  ;
	vec[1] =  	   cos(theta) * vtheta + sin(theta) * vr  ; */ 
	return vec;
}
...
{	
	...
	// name of result file
	string fileName = "EulerBarotropicStaggered_2DSphericalRiemann";
