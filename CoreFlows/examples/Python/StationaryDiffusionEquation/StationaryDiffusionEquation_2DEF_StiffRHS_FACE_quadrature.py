...
def StationaryDiffusionEquation_2DEF_StiffRHS_FACE_quadrature(mesh_file):
    ...
    sourceField = cdmath.Field("sourceField", cdmath.FACES, mesh, 1)
    exactSol    = cdmath.Field("exactSol"   , cdmath.NODES, mesh, 1)
    for i in range(mesh.getNumberOfFaces()):
        Fi = mesh.getFace(i)
        x = Fi.x()
        y = Fi.y()
    ...
    simSolution.writeVTK("simSolution_order2")
    exactSol.writeVTK("exactSol")
    sourceField.writeVTK("sourceField_faces")
...
StationaryDiffusionEquation_2DEF_StiffRHS_FACE_quadrature('../resources/squareWithTriangles.med')
