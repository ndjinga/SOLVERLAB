...
def convergence_StationaryDiffusion_2DFV_Neumann_Squares():
    start = time.time() 
    ### 2D FV Square mesh
    method='FV'
    BC = 'Neumann'
    ...
    plt.title('Plot over diagonal line for finite volumes with Neumann BC \n for Poisson problem on 2D square meshes')
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'convergence_StationaryDiffusion_2DFV_Neumann_Squares() : Make sure you use distinct meshes and at least two meshes'
    a=( a3*b1-a2*b2)/det
    b=(-a2*b1+a1*b2)/det
    ...
    plt.title('Convergence of finite volumes with Neumann BC \n for Poisson problem  on 2D square meshes')
    ...
    plt.title('Computational time of finite volumes with Neumann BC \n for Poisson problem on 2D square StrucMeshes')
