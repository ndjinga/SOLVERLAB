...
def convergence_StationaryDiffusion_2DFV_Neumann_RightTriangles(): 
    start = time.time()
    ...
    plt.title('Plot over diagonal line for finite volumes for the Poisson problem \n on 2D right triangles meshes with with Neumann BC')
    plt.savefig(mesh_name+"_2DFV_StatDiffusion_Neumann_PlotOverDiagonalLine.png")
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'convergence_StationaryDiffusion_2DFV_Neumann_RightTriangles() : Make sure you use distinct meshes and at least two meshes'
    a=( a3*b1-a2*b2)/det
    b=(-a2*b1+a1*b2)/det
    ...
    plt.title('Convergence of finite volumes  for the Poisson problem \n on 2D right triangles meshes with with Neumann BC')
    plt.savefig(mesh_name+"_2DFV_StatDiffusion_Neumann_ConvergenceCurve.png")
    ...
    plt.title('Computational time of finite volumes for the Poisson problem \n on 2D right triangles meshes with with Neumann BC')
    plt.savefig(mesh_name+"_2DFV_StatDiffusion_Neumann_ComputationalTime.png")
