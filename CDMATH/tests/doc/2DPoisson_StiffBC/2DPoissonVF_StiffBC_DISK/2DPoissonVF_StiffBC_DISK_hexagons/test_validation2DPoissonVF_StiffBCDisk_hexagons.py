...
def test_validation2DVF_StiffBC_hexagons():
    start = time.time()
    ...
    plt.title('Plot over diagonal line for finite volumes \n for Laplace operator on 2D disk with hexagon meshes')
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DVF_StiffBC_hexagons() : Make sure you use distinct meshes and at least two meshes'
    a=( a3*b1-a2*b2)/det
    b=(-a2*b1+a1*b2)/det
    
    print( "FV on 2D disk with hexagonal mesh : l2 scheme order is ", -a )
    assert abs(a+1.12)<0.01
...
if __name__ == """__main__""":
    test_validation2DVF_StiffBC_hexagons()
