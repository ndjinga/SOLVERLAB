# -*-coding:utf-8 -*
#===============================================================================================================================
# Name        : Résolution VF de l'équation de Poisson 2D -\triangle u = 0 sur un carré avec conditions aux limites de Dirichlet discontinues
...
def solve(my_mesh,filename,resolution, meshName, testColor):
    ...
    #=================================
    LS=cdmath.LinearSolver(Rigidite,RHS,100,1.E-6,"CG","LU")
    ...
    my_ResultField.writeVTK("FiniteVolumes2DPoissonStiffBC_SQUARE_"+meshName+str(nbCells))
    ...
    # save 2D picture
    PV_routines.Save_PV_data_to_picture_file("FiniteVolumes2DPoissonStiffBC_SQUARE_"+meshName+str(nbCells)+'_0.vtu',"ResultField",'CELLS',"FiniteVolumes2DPoissonStiffBC_SQUARE_"+meshName+str(nbCells))

    test_desc["Computational_time_taken_by_run"]=end-start
    test_desc["Absolute_error"]=l2_error
    test_desc["Relative_error"]=l2_error/l2_norm_sol_exacte

    with open('test_PoissonStiffBC'+str(my_mesh.getMeshDimension())+'D_VF_'+"SQUARE_"+str(nbCells)+ "Cells.json", 'w') as outfile:  
