import FiniteVolumes2DPoissonStiffBC_DISK
import matplotlib.pyplot as plt
import numpy as np
from math import log10, sqrt
import time, json

convergence_synthesis=dict(FiniteVolumes2DPoissonStiffBC_DISK.test_desc)

def test_validation2DVF_StiffBC_spiderWeb():
    start = time.time()
    #### 2D FV spider web mesh of a disk
    meshList=['diskWithSpiderWeb_1','diskWithSpiderWeb_2','diskWithSpiderWeb_3','diskWithSpiderWeb_4','diskWithSpiderWeb_5']
    meshType="Unstructured_triangles"
    testColor="Green"
    nbMeshes=len(meshList)
    error_tab=[0]*nbMeshes
    mesh_size_tab=[0]*nbMeshes
    mesh_path='../../../ressources/2DDiskSpiderWeb/'
    mesh_name='diskWithSpiderWeb'
    ...
    for filename in meshList:
        error_tab[i], mesh_size_tab[i], diag_data[i], min_tab[i], max_tab[i], time_tab[i] =FiniteVolumes2DPoissonStiffBC_DISK.solve_file(mesh_path+filename, resolution, mesh_name, testColor)
    ...
    plt.title('Plot over diagonal line for finite volumes \n for Laplace operator on 2D disk with spider web meshes')
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DVF_StiffBC_spiderWeb() : Make sure you use distinct meshes and at least two meshes'
    a=( a3*b1-a2*b2)/det
    b=(-a2*b1+a1*b2)/det
    
    print( "FV on 2D disk with spider web mesh : l2 scheme order is ", -a )
    assert abs(a+1.29)<0.01
...
if __name__ == """__main__""":
    test_validation2DVF_StiffBC_spiderWeb()
