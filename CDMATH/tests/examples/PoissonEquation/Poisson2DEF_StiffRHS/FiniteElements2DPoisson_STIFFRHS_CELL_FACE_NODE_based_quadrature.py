# -*-coding:utf-8 -*
#===============================================================================================================================
# Name        : Résolution EF avec quadrature utilisant cellules faces et noeuds de l'équation de Poisson 2D -\triangle u = f avec f raide
# Author      : Michaël Ndjinga
# Copyright   : CEA Saclay 2025
# Description : Utilisation d'une quadrature à 7 points pour la discrétisation de f sur un triangle ABC :
#                 \int f = 1/18 ( 2f(A)+2f(B)+2f(C) + 2f(AB)+2f(BC)+2f(AC) +6f(ABC) ) * |ABC|
...
for i in range(nbCells):
    ...
    x = Ci.x()
    y = Ci.y()
    rhsCell=right_hand_side(x, y)#Value at the center of mass for the right hand side integral evaluation
    ...
    for j in [nodeId0,nodeId1,nodeId2] :
        if boundaryNodes.count(j)==0 : #seuls les noeuds intérieurs contribuent au système linéaire (matrice de rigidité et second membre)
            ...
            Nj=my_mesh.getNode(j)
            x = Nj.x()
            y = Nj.y()
            rhsNode = right_hand_side(x, y)#Value at the node for the right hand side integral evaluation
            RHS[j_int]+=Ci.getMeasure()/9*rhsCell # contribution du centre de la cellule Ci à l'intégrale dans le triangle Ci du produit f x fonction de base
            RHS[j_int]+=Ci.getMeasure()/9*rhsNode # contribution du noeud Nj à l'intégrale dans le triangle Ci du produit f x fonction de base
            ...
            for k in [nodeId0,nodeId1,nodeId2] : 
                ...
                if j!=k :
                    ...
                    RHS[j_int]+=Ci.getMeasure()/18*rhsFace # # contribution de la face Fjk à l'intégrale dans le triangle Ci du produit f x fonction de base
