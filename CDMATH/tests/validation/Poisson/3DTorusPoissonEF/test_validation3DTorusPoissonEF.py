...
convergence_synthesis=dict(FiniteElements3DPoissonTorus.test_desc)
def test_validation3DTorusEF():
    ...
    time_tab=[0]*nbMeshes
    mesh_path='../../../ressources/3DTorus/'
    mesh_name='TorusWithTriangles'
    ...
    for filename in meshList:
        ...
        with open('./FiniteElementsOnTorusPoisson_PlotOnSortedLines'+meshType+str(mesh_size_tab[i])+'.csv') as f:
            lines = f.readlines()
    ...
    plt.title('Plot over slice circle for finite elements \n for Laplace operator on 3D torus meshes')
    plt.savefig(mesh_name+"_3DTorusPoissonFE_Slice.png")
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation3DTorusEF() : Make sure you use distinct meshes and at least two meshes'
    a=( a3*b1-a2*b2)/det
    b=(-a2*b1+a1*b2)/det
    ...
    plt.title('Convergence of finite elements for \n Laplace operator on 3D torus triangular meshes')
    plt.savefig(mesh_name+"_3DTorusPoissonFE_ConvergenceCurve.png")
    ...
    plt.title('Computational time of finite elements \n for Laplace operator on 3D torus triangular meshes')
    plt.savefig(mesh_name+"_3DTorusPoissonFE_ComputationalTime.png")
