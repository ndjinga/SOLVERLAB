...
def test_validation2DVF_right_triangles():
    start = time.time()
    ...
    for nx in meshList:
        my_mesh=cdmath.Mesh(0,1,nx,0,1,nx,1)
        my_mesh.setGroupAtPlan(1,0,eps,"Bord1")
        my_mesh.setGroupAtPlan(0,0,eps,"Bord2")
        my_mesh.setGroupAtPlan(1,1,eps,"Bord3")
        my_mesh.setGroupAtPlan(0,1,eps,"Bord4")
...
#        error_tab[i], mesh_size_tab[i], diag_data[i], min_sol_num, max_sol_num, time_tab[i] =FiniteVolumes2DPoisson_SQUARE.solve_file(mesh_path+filename,resolution,meshType,testColor)
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DVF_right_triangles() : Make sure you use distinct meshes and at least two meshes'
    a=( a3*b1-a2*b2)/det
    b=(-a2*b1+a1*b2)/det
    
    print( "FV on 2D right triangles mesh : scheme order is ", -a)
...
if __name__ == """__main__""":
    test_validation2DVF_right_triangles()
