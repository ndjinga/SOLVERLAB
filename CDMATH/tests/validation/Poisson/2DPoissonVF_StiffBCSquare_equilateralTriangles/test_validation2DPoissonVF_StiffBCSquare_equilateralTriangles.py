...
def test_validation2DVF_StiffBC_equilateralTriangles():
    start = time.time()
    #### 2D FV equilateral triangle mesh of a square
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DVF_StiffBC_equilateralTriangles() : Make sure you use distinct meshes and at least two meshes'
    a=( a3*b1-a2*b2)/det
    b=(-a2*b1+a1*b2)/det
    
    print( "FV on 2D square with equilateral triangle mesh : l2 scheme order is ", -a )
    assert abs(a+1.01)<0.01
...
if __name__ == """__main__""":
    test_validation2DVF_StiffBC_equilateralTriangles()
