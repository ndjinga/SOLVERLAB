...
convergence_synthesis=dict(FiniteElements2DPoissonStiffBC_SQUARE.test_desc)

def test_validation2DEF_StiffBC_Delaunay_triangles():
    start = time.time()
    #### 2D FE Delaunay triangle mesh of a square
    ...
    for filename in meshList:
        error_tab[i], mesh_size_tab[i], diag_data[i], min_tab[i], max_tab[i], time_tab[i] =FiniteElements2DPoissonStiffBC_SQUARE.solve(mesh_path+filename, resolution, mesh_name, testColor)
    ...
    plt.title('Plot over diagonal line for finite elements for Laplace operator \n on 2D square with Delaunay triangle meshes')
    ...
    print( "FE on 2D square with Delaunay triangle mesh : l2 scheme order is ", -a )
    ...
    plt.title('Convergence of finite elements for Laplace operator \n on 2D square with Delaunay triangle meshes')
