...
def test_validation2DEF_flat_cross_triangles():
    start = time.time()
    #### 2D FE flat cross triangle mesh
    #meshList=[5,9,15,21,31]
    meshList=['squareWithFlatCrossTriangles_00','squareWithFlatCrossTriangles_0','squareWithFlatCrossTriangles_1']#,'squareWithFlatCrossTriangles_2',,'squareWithFlatCrossTriangles_3','squareWithFlatCrossTriangles_4'
    mesh_path='./'
    ...
    plt.title('Plot over diagonal line for finite elements \n for Laplace operator on 2D flat cross triangle meshes')
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DEF_flat_cross_triangles() : Make sure you use distinct meshes and at least two meshes'
    a=( a3*b1-a2*b2)/det
    b=(-a2*b1+a1*b2)/det
    
    print( "FE on 2D flat cross triangle mesh : scheme order is ", -a )
    assert abs(a+1.377)<0.1
...
if __name__ == """__main__""":
    test_validation2DEF_flat_cross_triangles()
