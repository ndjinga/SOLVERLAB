import WaveSystemRiemannProblemComparison
import matplotlib
matplotlib.use("Agg")
...
def test_validationWaveSystemRiemannProblemComparison_mesh_refinement():
    start = time.time()
    ...
    for c0 in soundSpeedList:
        ntmax=int(nx/4)
        var_tot_upwind[i], var_tot_staggered[i], var_tot_centered[i] =WaveSystemRiemannProblemComparison.solve( a,b,nx,"1D","RegularGrid",cfl,c0,ntmax)
        i=i+1
    ...
    plt.legend()
    plt.savefig(mesh_name+"_WaveSystemTotalVariationSoundSpeedIncrease.png")
    
    
if __name__ == """__main__""":
    test_validationWaveSystemRiemannProblemComparison_mesh_refinement()
