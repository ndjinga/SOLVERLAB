...
def test_validation2DWaveSystemCenteredHexagons(bctype,scaling):
    start = time.time()
    #### 2D hexagonal mesh
    ...
    for filename in meshList:
        ...
        assert max_vel[i]>0.0001 and max_vel[i]<1.1
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DWaveSystemCenteredHexagonsFV() : Make sure you use distinct meshes and at least two meshes'
...
if __name__ == """__main__""":
    if len(sys.argv) >2 :
        ...
        test_validation2DWaveSystemCenteredHexagons(bctype,scaling)
    else :
        raise ValueError("test_validation2DWaveSystemCenteredHexagons.py expects a mesh file name and a scaling parameter")
