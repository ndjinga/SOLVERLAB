...
#    for filename in meshList:
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DWaveSystemSquaresFVStaggered_squares() : Make sure you use distinct meshes and at least two meshes'
    ...
    if(scaling==0):
        print("FV Staggered on 2D square meshes : scheme order for pressure without scaling is ", -ap)
    else:
        print("FV Staggered on 2D square meshes : scheme order for pressure with    scaling is ", -ap)
    ...
    if(scaling==0):
        print("FVStaggered on 2D square meshes : scheme order for velocity without scaling is ", -au)
    else:
        print("FVStaggered on 2D square meshes : scheme order for velocity with    scaling is ", -au)
    ...
    if(scaling==0):
        convergence_synthesis["Numerical_method_name"]="Staggered no scaling"
    else:
        convergence_synthesis["Numerical_method_name"]="Staggered scaling"			
    ...
    with open('Convergence_WaveSystem_2DFV_Staggered_'+mesh_name+'.json', 'w') as outfile:  
        json.dump(convergence_synthesis, outfile)

if __name__ == """__main__""":
    if len(sys.argv) >1 :
        scaling = int(sys.argv[1])
        test_validation2DWaveSystemStaggered_squares(scaling)
    else :
        raise ValueError("test_validation2DWaveSystemStaggeredSquares.py expects a mesh file name")
