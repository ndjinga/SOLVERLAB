...
def test_validation2DWaveSystemPStag_brickwall(scaling):
    start = time.time()
    #### 2D brick wall meshes
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DWaveSystemBrickWallFVPStag() : Make sure you use distinct meshes and at least two meshes'
    ...
    if(scaling==0):
        print("FV PStag on 2D brick wall meshes : scheme order for pressure without scaling is ", -ap)
    else:
        print("FV PStag on 2D brick wall meshes : scheme order for pressure with    scaling is ", -ap)
    ...
    if(scaling==0):
        print("FV PStag on 2D brick wall meshes : scheme order for velocity without scaling is ", -au)
    else:
        print("FV PStag on 2D brick wall meshes : scheme order for velocity with scaling is ", -au)
...
if __name__ == """__main__""":
    if len(sys.argv) >1 :
        ...
        test_validation2DWaveSystemPStag_brickwall(scaling)
    else :
        test_validation2DWaveSystemCentered_brickwall(2)
