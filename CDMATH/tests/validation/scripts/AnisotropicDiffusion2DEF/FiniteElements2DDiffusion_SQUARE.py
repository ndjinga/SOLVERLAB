# -*-coding:utf-8 -*
#===============================================================================================================================
# Name        : Résolution EF de l'équation de Diffusion 2D -(\partial_{xx} u + K \partial_{yy} u) = f sur un carré avec conditions aux limites de Dirichlet u=0
...
test_desc["Global_name"]="FE simulation of the 2D Anisotropic diffusion equation"
...
def solve(filename,resolution,meshType, testColor):
    ...
    for i in range(nbNodes):
        ...
        my_RHSfield[i]=(1+K)*pi*pi*sin(pi*x)*sin(pi*y)#mettre la fonction definie au second membre de l'edp
            ...
            if boundaryNodes.count(j)==0 : #seuls les noeuds intérieurs contribuent au système linéaire (matrice de rigidité et second membre)
                ...
                for k in [nodeId0,nodeId1,nodeId2] : 
                    if boundaryNodes.count(k)==0 : #seuls les noeuds intérieurs contribuent à la matrice du système linéaire
                        ...
                        Rigidite.addValue(j_int,k_int,GradShapeFuncs[j]*(DiffusionMatrix*GradShapeFuncs[k])/Ci.getMeasure())
    ...
    my_ResultField.writeVTK("FiniteElements2DAnisotropicDiffusion_SQUARE_"+meshType+str(nbNodes))
    
    print("Numerical solution of 2D anisotropic diffusion on a square using finite elements done")
    ...
    erreur_abs=0
    for i in range(nbNodes) :
        if erreur_abs < abs(my_RHSfield[i]/((1+K)*pi*pi) - my_ResultField[i]) :
            erreur_abs = abs(my_RHSfield[i]/((1+K)*pi*pi) - my_ResultField[i])
    ...
    # save 2D picture
    PV_routines.Save_PV_data_to_picture_file("FiniteElements2DAnisotropicDiffusion_SQUARE_"+meshType+str(nbNodes)+'_0.vtu',"ResultField",'NODES',"FiniteElements2DAnisotropicDiffusion_SQUARE_"+meshType+str(nbNodes))
    
    test_desc["Computational_time_taken_by_run"]=end-start
    test_desc["Absolute_error"]=erreur_abs
    test_desc["Relative_error"]=erreur_abs/max_abs_sol_exacte

    with open('test_Diffusion'+str(my_mesh.getMeshDimension())+'D_EF_'+"SQUARE_"+meshType+str(nbCells)+ "Cells.json", 'w') as outfile:  
