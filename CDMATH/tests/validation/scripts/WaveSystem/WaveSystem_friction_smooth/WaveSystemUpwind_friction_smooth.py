...
#                \partial_t p + c^2 \div q = 8*c0^2*pi^2*sin(2*pi*x)*sin(2*pi*y)
...
#               Initialisation par un champ nul
...
def WaveSystemVF(ntmax, tmax, cfl, my_mesh, output_freq, meshName, resolution,scaling,test_bc):
    ...
    if(meshName.find("square")==-1 and meshName.find("Square")==-1 and meshName.find("cube")==-1 and meshName.find("Cube")==-1):
        ...
        raise ValueError("Mesh name should contain substring square or cube to use wave system with smooth source term")
    ...
    if(it>=ntmax):
        ...
        return delta_press   , delta_v.maxVector()[0], nbCells, time, it, velocity_field.getNormEuclidean().max(), diag_data_press, diag_data_vel
    else:
