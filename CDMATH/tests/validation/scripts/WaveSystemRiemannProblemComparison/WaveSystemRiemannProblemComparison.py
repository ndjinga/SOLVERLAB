...
# Linear Wave system
# d rho/d t + d q/d x =0
# d q/d t   + d p/d x = 0, where q = rho*u and p = c^2*rho
...
def WaveSystemSchemeComparison(ntmax, tmax, cfl, a,b,nx, output_freq, meshName, c0):
    dim=1
    nbComp=dim+1
    nbCells = nx
    dt = 0.
    ...
    print("Starting computation of the linear wave system with various schemes …")
    ...
    for j in range(nbCells-1):
        lj =   Un_upwind[(j  )*nbComp+0]*c0 + Un_upwind[(j  )*nbComp+1]
        ljp1 = Un_upwind[(j+1)*nbComp+0]*c0 + Un_upwind[(j+1)*nbComp+1]
        var_tot_upwind[it]+=abs(ljp1-lj)
        lj =   Un_staggered[(j  )*nbComp+0]*c0 + Un_staggered[(j  )*nbComp+1]
        ljp1 = Un_staggered[(j+1)*nbComp+0]*c0 + Un_staggered[(j+1)*nbComp+1]
        var_tot_staggered[it]+=abs(ljp1-lj)
        lj =   Un_centered[(j  )*nbComp+0]*c0 + Un_centered[(j  )*nbComp+1]
        ljp1 = Un_centered[(j+1)*nbComp+0]*c0 + Un_centered[(j+1)*nbComp+1]
    ...
    metadata = dict(title="Scheme comparison for the 1D isothermal wave System", artist = "CEA Saclay", comment="Shock tube")
    writer=FFMpegWriter(fps=10, metadata=metadata, codec='h264')
    with writer.saving(fig, "1D wave_System_Scheme_Comparison_"+str(nx)+"Cells_soundSpeed"+str(c0)+""+".mp4", ntmax):
        writer.grab_frame()
        plt.savefig("WaveSystem"+str(dim)+"D_Scheme_Comparison"+meshName+"_0_"+str(nx)+"Cells_soundSpeed"+str(c0)+""+".png")
        ...
        while (it<ntmax and time <= tmax ):
            ...
            if(it==1 or it%output_freq==0 or it>=ntmax or time >=tmax):
                ...
                #print("Upwind : Last linear system converged in ", LS_upwind.getNumberOfIter(), " GMRES iterations")

                plt.savefig("WaveSystem"+str(dim)+"D_Scheme_Comparison"+meshName+"_"+str(it)+"_"+str(nx)+"Cells_soundSpeed"+str(c0)+".png")
                print
...
def solve( a,b,nx, meshName, meshType, cfl,c0,ntmax):
    ...
    return WaveSystemSchemeComparison(ntmax, tmax, cfl, a,b,nx, output_freq, meshName,c0)
