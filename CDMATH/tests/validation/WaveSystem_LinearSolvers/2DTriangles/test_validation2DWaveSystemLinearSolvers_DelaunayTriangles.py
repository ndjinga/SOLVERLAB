...
def test_validation2DWaveSystem_LinearSolver_triangles(num_scheme, lin_solver,cfl, mesh_folder):
    start = time.time()
...
if __name__ == """__main__""":
    if len(sys.argv) >4 :
        ...
        test_validation2DWaveSystem_LinearSolver_triangles(num_scheme, lin_solver,cfl, mesh_folder)
    else :
        raise ValueError("test_validation2DWaveSystem_LinearSolver_triangles.py expects a numerical method, a linear solver, a cfl number, and a mesh folder path")
