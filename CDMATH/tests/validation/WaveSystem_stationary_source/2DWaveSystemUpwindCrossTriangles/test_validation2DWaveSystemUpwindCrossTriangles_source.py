...
def test_validation2DWaveSystemSourceUpwindCrossTriangles(bctype,scaling):
    start = time.time()
    #### 2D cross triangles mesh
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DWaveSystemSourceUpwindCrossTrianglesFV() : Make sure you use distinct meshes and at least two meshes'
...
if __name__ == """__main__""":
    if len(sys.argv) >2 :
        ...
        test_validation2DWaveSystemSourceUpwindCrossTriangles(bctype,scaling)
    else :
        raise ValueError("test_validation2DWaveSystemSourceUpwindCrossTriangles.py expects a mesh file name and a scaling parameter")
