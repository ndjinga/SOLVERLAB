...
def test_validation2DWaveSystemUpwindBrickWall(bctype,scaling):
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DWaveSystemSourceUpwindBrickWallFV() : Make sure you use distinct meshes and at least two meshes'
...
if __name__ == """__main__""":
    if len(sys.argv) >2 :
        ...
        test_validation2DWaveSystemSourceUpwindBrickWall(bctype,scaling)
    else :
        raise ValueError("test_validation2DWaveSystemSourceUpwindBrickWall.py expects a mesh file name and a scaling parameter")
