...
def test_validation2DWaveSystemSourcePStag_squares(scaling):
    start = time.time()
    #### 2D square mesh
    ...
    for nx in meshList:
        ...
        error_p_tab[i], error_u_tab[i], mesh_size_tab[i], t_final[i], ndt_final[i], max_vel[i], diag_data_press[i], diag_data_vel[i], time_tab[i], cond_number[i] =WaveSystemPStag.solve(my_mesh,"square"+str(nx)+'x'+str(nx),resolution,scaling,meshType,testColor,cfl,"Periodic",True)
        #error_p_tab[i], error_u_tab[i], mesh_size_tab[i], t_final[i], ndt_final[i], max_vel[i], diag_data_press[i], diag_data_vel[i], time_tab[i], cond_number[i] =WaveSystemPStag.solve_file(mesh_path+filename, mesh_name, resolution,scaling,meshType,testColor,cfl,True)
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DWaveSystemSourceSquaresFVPStag_squares() : Make sure you use distinct meshes and at least two meshes'
    ...
    plt.savefig(mesh_name+"2DWaveSystemSourcePStag_"+"scaling"+str(scaling)+"_ComputationalTimeSquares.png")
...
if __name__ == """__main__""":
    if len(sys.argv) >1 :
        ...
        test_validation2DWaveSystemSourcePStag_squares(scaling)
    else :
        test_validation2DWaveSystemSourcePStag_squares(2)
