...
def test_validation2DWaveSystemSourcePStagDeformedQuadrangles(bctype,scaling):
    start = time.time()
    ...
    for filename in meshList:
        error_p_tab[i], error_u_tab[i], mesh_size_tab[i], t_final[i], ndt_final[i], max_vel[i], diag_data_press[i], diag_data_vel[i], time_tab[i] =WaveSystemSourcePStag.solve_file(mesh_path+filename, mesh_name, resolution, scaling,meshType,testColor,cfl,bctype,True)
    ...
    plt.title('Plot over diagonal line for stationary wave system \n with PStag scheme on 2D deformed quadrangular meshes')
    plt.savefig(mesh_name+'_Pressure_2DWaveSystemSourcePStag_'+"PlotOverDiagonalLine.png")
    plt.close()
    ...
    plt.savefig(mesh_name+"_Velocity_2DWaveSystemSourcePStag_"+"PlotOverDiagonalLine.png")    
    plt.close()
    ...
    plt.savefig(mesh_name+"_2DWaveSystemSourcePStag_"+"TimeSteps.png")
    ...
    plt.savefig(mesh_name+"_2DWaveSystemSourcePStag_"+"TimeFinal.png")
    ...
    plt.savefig(mesh_name+"_2DWaveSystemSourcePStag_"+"MaxVelNorm.png")
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DWaveSystemSourcePStagDeformedQuadranglesFV() : Make sure you use distinct meshes and at least two meshes'
    ...
    plt.savefig(mesh_name+"_Pressure_2DWaveSystemSourcePStag_"+"ConvergenceCurve.png")
    ...
    plt.title('Convergence of finite volumes for the stationary Wave System \n with PStag scheme on 2D deformed quadrangular meshes')
    plt.savefig(mesh_name+"_Velocity_2DWaveSystemSourcePStag_"+"ConvergenceCurve.png")
    ...
    plt.savefig(mesh_name+"_2DWaveSystemSourcePStag_ComputationalTime.png")
...
if __name__ == """__main__""":
    if len(sys.argv) >2 :
        ...
        test_validation2DWaveSystemSourcePStagDeformedQuadrangles(bctype,scaling)
    else :
        raise ValueError("test_validation2DWaveSystemSourcePStagDeformedQuadrangles.py expects a mesh file name and a scaling parameter")
