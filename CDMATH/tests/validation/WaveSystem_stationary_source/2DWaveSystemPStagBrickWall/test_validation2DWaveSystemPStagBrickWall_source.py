...
def test_validation2DWaveSystemSourcePStag_brickwall(scaling):
    start = time.time()
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DWaveSystemSourceBrickWallFVPStag() : Make sure you use distinct meshes and at least two meshes'
    ...
    plt.savefig(mesh_name+"2DWaveSystemSourcePStag_"+"scaling"+str(scaling)+"_ComputationalTimeBrickWall.png")
...
if __name__ == """__main__""":
    if len(sys.argv) >1 :
        ...
        test_validation2DWaveSystemSourcePStag_brickwall(scaling)
    else :
        test_validation2DWaveSystemSourcePStag_brickwall(2)
