...
def test_validation2DWaveSystemSourcePStagHexagons(bctype,scaling):
    start = time.time()
    #### 2D hexagonal mesh
    ...
    for filename in meshList:
        error_p_tab[i], error_u_tab[i], mesh_size_tab[i], t_final[i], ndt_final[i], max_vel[i], diag_data_press[i], diag_data_vel[i], time_tab[i] =WaveSystemPStag.solve_file(mesh_path+filename, mesh_name, resolution,scaling, meshType,testColor,cfl,bctype,True)
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DWaveSystemSourcePStagHexagonsFV() : Make sure you use distinct meshes and at least two meshes'
...
if __name__ == """__main__""":
    if len(sys.argv) >2 :
        ...
        test_validation2DWaveSystemSourcePStagHexagons(bctype,scaling)
    else :
        raise ValueError("test_validation2DWaveSystemSourcePStagHexagons.py expects a mesh file name and a scaling parameter")
