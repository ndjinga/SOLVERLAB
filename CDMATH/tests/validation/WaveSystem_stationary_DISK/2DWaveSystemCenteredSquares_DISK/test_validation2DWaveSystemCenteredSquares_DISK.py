...
def test_validation2DWaveSystemCenteredSquares_DISK(bctype,scaling):
    start = time.time()
    #### 2D DISK mesh
    ...
    testColor="Orange : if BC changed to Neuman, KSP diverge reason -p (nan or inf)"
    nbMeshes=len(meshList)
    error_p_tab=[0]*nbMeshes
    error_u_tab=[0]*nbMeshes
    mesh_size_tab=[0]*nbMeshes
    mesh_name='diskWithSquares'
    ...
    plt.title('Plot over diagonal line for stationary wave system \n with centered scheme on 2D DISK with square meshes')
    plt.savefig(mesh_name+'_Pressure_2DWaveSystemCentered_DISK_'+"PlotOverDiagonalLine.png")
    plt.close()
    ...
    plt.savefig(mesh_name+"_Velocity_2DWaveSystemCentered_DISK_"+"PlotOverDiagonalLine.png")    
    plt.close()
    ...
    plt.savefig(mesh_name+"_2DWaveSystemCentered_DISK_"+"TimeSteps.png")
    ...
    plt.savefig(mesh_name+"_2DWaveSystemCentered_DISK_"+"TimeFinal.png")
    ...
    plt.savefig(mesh_name+"_2DWaveSystemCentered_DISK_"+"MaxVelNorm.png")
    ...
    det=a1*a3-a2*a2
    assert det!=0, 'test_validation2DWaveSystemCentered_DISK_squares() : Make sure you use distinct meshes and at least two meshes'
    ...
    plt.savefig(mesh_name+"_Pressure_2DWaveSystemCentered_DISK_"+"ConvergenceCurve.png")
    ...
    plt.title('Convergence of finite volumes for the stationary Wave System\n with centered scheme  on 2D DISK with square meshes')
    plt.savefig(mesh_name+"_Velocity_2DWaveSystemCentered_DISK_"+"ConvergenceCurve.png")
    ...
    plt.savefig(mesh_name+"_2DWaveSystemCentered_DISK_ComputationalTimeSquares.png")
    ...
    convergence_synthesis["Initial_data"]="Disk vortex (Constant pressure, divergence free velocity)"
    ...
    with open('Convergence_WaveSystem_2DFV_Centered_DISK_'+mesh_name+'.json', 'w') as outfile:  
...
if __name__ == """__main__""":
    if len(sys.argv) >2 :
        ...
        test_validation2DWaveSystemCenteredSquares_DISK(bctype,scaling)
    else :
        raise ValueError("test_validation2DWaveSystemCenteredSquares_DISK.py expects a mesh file name and a scaling parameter")
