...
Print debug messages in sys.stderr.
Show pretty print debug representation from instances of some classes

| Warning: supposedly show messages in development phase, not production
| 
| Usage:
| >> import debug as DBG
...
_developers = ["christian", "wambeke", "ym268439"] # yourself, if you want ...
...
class OutStream(IOX.StringIO):
    ...
    def close(self):
      ...
      IOX.StringIO.close(self)
    
class InStream(IOX.StringIO):
